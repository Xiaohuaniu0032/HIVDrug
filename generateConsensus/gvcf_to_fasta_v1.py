    ...
    with FastaWriter(output_fasta_path) as f_fasta_out:
        ...
        for gvcf_dict in gvcf_lines:
            ...
            # Get sequence from variant calling line
                ...
                # Determine the allele idx to be used for the FASTA sequence, change the major allele only value according to min_non_hpindel_var_freq. revised by ZSQ, 7/5/22
                use_allele_idx_list = determine_allele_idx_to_use(gvcf_dict, major_allele_only)
                num_alt = len(gvcf_dict['ALT'])
                fdvr_str = gvcf_dict['INFO'].get('FDVR', ','.join('.'*num_alt))
                is_hp_indel = [fdvr == '0' for fdvr in fdvr_str.split(',')]
                af_str = gvcf_dict['INFO'].get('AF', ','.join('0'*num_alt))
                af_list = [float(af) for af in af_str.split(',')]
                for allele_idx in tuple(use_allele_idx_list):
                    if not allele_idx:
                        continue
                    alt_idx = allele_idx - 1
                    my_min_var_freq_thresold = min_hpindel_var_freq if is_hp_indel[alt_idx] else min_non_hpindel_var_freq
                    # change the  major_allele_only to 1 if its AF <  threshold
                    if af_list[alt_idx] < my_min_var_freq_thresold or af_list[alt_idx] > 1-my_min_var_freq_thresold:
                        major_allele_only = 1
                new_use_allele_idx_list = determine_allele_idx_to_use(gvcf_dict, major_allele_only)
                ...
                do_variant_filtering_inplace(new_use_allele_idx_list, gvcf_dict, min_non_hpindel_var_freq, min_hpindel_var_freq, min_dp)
